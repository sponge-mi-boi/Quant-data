...
def main():
    prices=pd.read_parquet(PROJECT/'data'/'processed'/'close_1d_10y.parquet').iloc[:1020]
    market=prices['SPY'].pct_change().fillna(0.); assets=prices.drop(columns='SPY'); returns=assets.pct_change().fillna(0.)
    screen=pd.read_csv(ROOT/'work'/'cointegration_all_pairs_screen.csv'); screen['q_value']=benjamini_hochberg(screen.p_value.to_numpy()); survivors=screen[screen.q_value<=.05].sort_values('q_value')
    training=[]
    ...
    path=ROOT/'outputs'/'cointegration_one_screen_fast_summary.json'; path.write_text(json.dumps(output,indent=2,allow_nan=False),encoding='utf-8'); print(json.dumps(output,indent=2,allow_nan=False))
