...
def get_time_period(args, type_='Close', freq='1d',
    ...
    if time_peri is None or len(time_peri) != 2:
        raise ValueError('time_peri must be a (start, end) pair')
    start, end = (int(time_peri[0]), int(time_peri[1]))
...
def get_yf(per, int_, stocks=tuple(), type_='Close') -> None:
    ...
    if type_ in ['Close', 'Volume', 'High', 'Low']:
        ...
        raise ValueError("type_ must be one of 'Close', 'Volume', 'High', or 'Low'")
