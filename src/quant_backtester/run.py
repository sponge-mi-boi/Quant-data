...
def run():
    ...
    for x in range(0, 2  ):
        ...
        name =  str ( path ) + '/artifacts/mv/'  + 'results_training_' + str(time_period) + '.parquet'
    ...
    for x in range( 0 , 2  ):
        ...
        name = str(path) + '/artifacts/cointegration/results_training_' + str(time_period) + '.parquet'
    ...
    for x in range( 0   , 2  ):
        ...
        name = str(path) + '/artifacts/cointegration/results_validating_' + str(time_period) + '.parquet'

        runner_multiple(pd.DataFrame(index=[tuple(x) for x in stck_list]), [time_period],
                        port_sim, init_money=1000, strat_class='cointegration',
                        inputs=None, num_processes=16,  output_metrics=['Total Return', 'Sharpe', 'Alpha', 'Number of Trades'],
                        freq='d', graphs=False, parameters_=[1.5, 26]).to_parquet(name)

        stck_list = pd.read_parquet(name) .sort_values(by=[str(time_period)  + ' Number of Trades']) .tail().index[:]
